...
;	メニューボタン非表示
	...
	tf.config_x       = [1040, 614,　670, 726, 782, 839, 887, 940, 996, 1052, 1108]; // X座標（共通）

	tf.config_y_bgm   = 180; // BGMのY座標
	tf.config_y_se    = 247; // SEのY座標
	tf.config_y_ch    = 314; // テキスト速度のY座標
	tf.config_y_auto  = 383; // オート速度のY座標
...
;	テキスト表示速度のサンプルに使用するメッセージレイヤの設定
	[position layer="message1" left="150" top="460" width="980" height="100" margint="2" marginl="30" page="fore" visible="true" opacity="0"]
...
;	サンプルテキストを表示する
	[emb exp="tf.text_sample"]
		...
		[iscript]
		$(".current_span").css("color","#211A1A"); // サンプルテキストのフォントカラーを指定
